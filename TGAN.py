...
class TRIGAN(GANBase):
    ...
    def _build_classifier(self, tensor=None, training=False, init=False):
        if tensor is not None:
            ...
            input = self.input_labeled_x_zca
    ...
    def _build_generator(self, tensor=None, label=None, training=False, batch_norm=None, init=False):
        ...
        if label is None:
            if self.input_y_g is None:
                ...
                label = get_one_hot(np.repeat(np.tile(self.n_class), (batch_size / self.m_class) + 1),
                                    depth=self.n_class)
    ...
    def _build_metrics(self):
        ...
        with tf.name_scope('metrics') as scope:
            consist_logits = self._build_classifier(self.input_labeled_x_zca, training=training)
    ...
    def _build_loss(self, label_strength=1., training=False):
        ...
        fake_y_c = self._build_classifier(self.input_x_c_zca, training=training)
        true_logits = self._build_classifier(self.input_labeled_x_zca, training=training)

        #TODO  input_x_c_zca->input_x_c
        unlabeled_label_d, unlabeled_logits_d = self._build_discriminator(self.input_x_c_zca, fake_y_c, training=training)
        ...
        with tf.name_scope('loss') as scope:
            ...
            lossC_c = tf.reduce_mean(
                tf.nn.softmax_cross_entropy_with_logits(logits=true_logits, labels=self.input_labeled_y))
            lossC = lossC_c
    ...
    def train(self, labeled_x, labeled_y, unlabeled_x, test_x, test_y, n_ptepoch=0, n_epochs=None, n_batch=128,
