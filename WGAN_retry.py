...
class WGAN(object):
    ...
    def __init__(self, name=None, log_and_save=True, debug=False):
        ...
        self.batch_norm_D = False #in wgan_gp, must be false
    ...
    def _build_generator_mlp(self,tensor=None,training=False):
        if tensor is None:
        ...
        with tf.variable_scope('generator', reuse=tf.AUTO_REUSE) as scope:
            mlp_node_count=512*3
            for layers in range(3):
                with tf.variable_scope('layers.{0}-{1}'.format(layers,mlp_node_count)):
                    tensor=tf.nn.leaky_relu(bn(tf.layers.dense(tensor,units=mlp_node_count,kernel_initializer=init_normal(), kernel_regularizer=tf.contrib.layers.l2_regularizer(2.5e-5))))
            with tf.variable_scope('final.{0}-{1}'.format(3,self.n_pixel*self.n_pixel*self.n_channel)):
                tensor=bn(tf.layers.dense(tensor,units=self.n_pixel*self.n_pixel*self.n_channel,kernel_initializer=init_normal(),kernel_regularizer=tf.contrib.layers.l2_regularizer(2.5e-5),activation=tf.sigmoid))
            return tf.reshape(tensor,shape=[-1,self.n_pixel,self.n_pixel,3])
    ...
    def _build_generator_contrib(self, tensor=None,training=False):
        ...
        with tf.variable_scope('generator', reuse=tf.AUTO_REUSE) as scope:
            ...
            with tf.variable_scope('initial.{0}-{1}'.format(self.n_noise, n_filt)):
                tensor = tf.nn.relu(bn(tf.layers.dense(tf.reshape(tensor, [-1,self.n_noise]), units=4 * 4 * n_filt,
                                            kernel_initializer=init_normal(0.02),use_bias=True,
                                            name='dense'), name='bn'))
            ...
            for layers in range(3):
                with tf.variable_scope('pyramid.{0}-{1}'.format(n_filt, n_filt // 2)):
                    tensor = tf.nn.relu(bn(tfcl.conv2d_transpose(tensor, n_filt // 2, [5,5], [2,2], 'SAME',activation_fn=None,
                                                                 weights_initializer=init_normal(0.02),biases_initializer=init_normal(0.01)),name='bn'))
                n_filt //= 2
            # final layer
            with tf.variable_scope('final.{0}-{1}'.format(n_filt, self.n_channel)):
                tensor = tfcl.conv2d_transpose(tensor, self.n_channel, [5,5], [2,2], 'SAME',activation_fn=tf.tanh,
                                               weights_initializer=init_normal(0.02),biases_initializer=init_normal(0.01))
            return tensor
    def _build_generator(self, tensor=None,training=False):
        if tensor is None:
            print('NONE VALUE INPUT TO G.')
            raise
        if self.batch_norm_G:
        ...
        with tf.variable_scope('generator', reuse=tf.AUTO_REUSE) as scope:
            ...
            use_bias=False
            with tf.variable_scope('initial.{0}-{1}'.format(self.n_noise, n_filt)):
                tensor = bn(tf.layers.dense(tf.reshape(tensor, [-1,self.n_noise]), units=4 * 4 * n_filt,
            ...
            for layers in range(3):
                with tf.variable_scope('pyramid.{0}-{1}'.format(n_filt, n_filt // 2)):
                    tensor = tf.nn.relu(bn(tf.layers.conv2d_transpose(tensor, n_filt // 2, 5, 2, 'same',
                                                                      use_bias=use_bias,
            ...
            # final layer
            with tf.variable_scope('final.{0}-{1}'.format(n_filt, self.n_channel)):
                tensor = tf.layers.conv2d_transpose(tensor, self.n_channel, 5, 2, 'same', use_bias=use_bias,activation=tf.tanh,
                                                    kernel_initializer=init_normal(0.01),
                                                    name='conv_t')
            return tensor
    ...
    def _build_discriminator(self, tensor=None, training=False):
        ...
        with tf.variable_scope('discriminator', reuse=tf.AUTO_REUSE) as scope:
            ...
            #    bn = tf.identity
            # initial layer
            with tf.variable_scope('pyramid.{0}-{1}'.format(self.n_channel, n_filt)):
                tensor = tf.nn.leaky_relu(tf.layers.conv2d(tensor, n_filt, 5, 2, 'same',use_bias=use_bias,kernel_initializer=init_normal(),bias_initializer=init_normal(0.01),name='conv'))
            n_filt *= 2
            # downscaling layers
            for layers in range(3):
            ...
            with tf.variable_scope('final.{0}-{1}'.format(tensor.shape[-1], 1)):
                ...
                tensor = tf.nn.leaky_relu(
                    tf.layers.conv2d(tensor, n_filt, 4, 2, 'same', use_bias=use_bias,kernel_initializer=init_normal(),bias_initializer=init_normal(0.01), name='conv'))
    ...
    def _build_loss(self, training=False, penalty_strength=10):
        # networks
        fake = self._build_generator_contrib(self.input_z,training=training)
        epsilon = tf.reshape(tf.tile(tf.random_uniform((tf.shape(self.input_z)[0],1), 0.0, 1.0),[1,self.n_pixel*self.n_pixel*self.n_channel]),[-1,self.n_pixel,self.n_pixel,self.n_channel])
        ...
        with tf.name_scope('define_lossD') as scope:
            ...
            norm_gradD = tf.sqrt(tf.reduce_sum(tf.square(tf.gradients(semifake_logits, semifake)[0]), reduction_indices=[-1]))
        ...
        if training:
            tf.summary.image('fake', self.merge_image(fake),max_outputs=1)
    ...
    def train(self, trainx, testx, n_epochs=25, n_batch=64, learning_rate=1e-4,penalty_strength=10, n_updated_d=5):
                ...
                for batch_index, n_batch_actual in batcher(n_train, n_batch):
                    ...
                    _, temp = sess.run([adamD, lossD],
                                       {self.input_x: trainx[batch_index:batch_index + n_batch_actual],
                                        self.input_z: np.random.uniform(-1,1,[n_batch_actual, self.n_noise]).astype(
                                            np.float32)})
                    ...
                    if t_index % n_updated_d == 0:
                        ...
                        _, summary, step, temp = sess.run([adamG, merged_summary, global_step, lossG],
                                                          {self.input_x: trainx[
                                                           ...
                                                           self.input_z: np.random.uniform(-1,1,[n_batch_actual, self.n_noise]).astype(
                                            np.float32)})
                        _,_,_= sess.run([check_op,global_step_inc,decay_learning_rate],{self.input_x: trainx[
                                                                         batch_index:batch_index + n_batch_actual],
                                                           self.input_z: np.random.uniform(-1,1,[n_batch_actual, self.n_noise]).astype(
                                            np.float32)})
...
if __name__=='__main__':
    from data_process import load_cifar10_all,load_cifar10_rgb,load_celebA,load_mnist
    train_data,_, test_data, _=load_celebA()
    FLAGS = tf.app.flags.FLAGS
    tf.app.flags.DEFINE_boolean('debug', False, 'activate tfdb')
    tf.app.flags.DEFINE_integer('ep', 25, 'training epochs')
    debug = FLAGS.debug
    epochs = FLAGS.ep
    w_gan = WGAN(debug=debug)
    start=time()
    w_gan.train(train_data, test_data, n_epochs=epochs,n_updated_d=5,learning_rate=1e-4)
