...
class WCATGAN(object):
    ...
    def label_weights(self,label=None,source='real',mode='lossD'):
        #i dont think mode is needed
        l_shape=tf.shape(label)
        #l_shape=tf.Print(l_shape,[l_shape])
        if mode=='lossD':
            if source=='real':
                label_w=tf.concat([((label*self.n_class)-1)/self.n_class,tf.zeros(tf.stack([l_shape[0],1]))+1],1)
            elif source=='fake':
                label_w = tf.concat([((label*self.n_class)-1)/self.n_class,tf.ones(tf.stack([l_shape[0],1]))-1],1)
        elif mode=='label':
            if source=='real':
                label_w=tf.concat([label,tf.zeros(tf.stack([l_shape[0],1]))+1],1)
            elif source=='fake':
                label_w = tf.concat([label, tf.zeros(tf.stack([l_shape[0], 1])) + 10], 1)
        return label_w

    def _build_generator_contrib(self, tensor=None,label=None, training=False):
        if tensor is None:
            print('NONE VALUE INPUT TO G.')
            raise
        if label is None:
            print('NONE LABEL INPUT TO G.')
            raise
        ...
        #merge noise with class
        tensor = tf.concat([tensor, label], 1)
        with tf.variable_scope('generator', reuse=tf.AUTO_REUSE) as scope:
            n_filt = 512
            use_bias = True
            with tf.variable_scope('initial.{0}-{1}'.format(self.n_noise, n_filt)):
                tensor = tf.layers.dense(tf.reshape(tensor, [-1, self.n_noise+self.n_class]), units=4 * 4 * n_filt,
            ...
            for layers in range(2):
                with tf.variable_scope('pyramid.{0}-{1}'.format(n_filt, n_filt // 2)):
                    tensor = tf.nn.relu(
                        bn(tfcl.conv2d_transpose(tensor, n_filt // 2, [5, 5], [2, 2], 'SAME', activation_fn=None,
            ...
            with tf.variable_scope('final.{0}-{1}'.format(n_filt, self.n_channel)):
                tensor = tfcl.conv2d_transpose(tensor, self.n_channel, [5, 5], [2, 2], 'SAME',
                                               activation_fn=tf.tanh,
                                               weights_initializer=init_normal(0.02), biases_initializer=None)
            return tensor

    def _build_discriminator(self, tensor=None, mode=None,training=False):
        if tensor is None:
        ...
        with tf.variable_scope('discriminator', reuse=tf.AUTO_REUSE) as scope:
            ...
            with tf.variable_scope('final.{0}-{1}'.format(tensor.shape[-1], n_filt)):
                ...
                # ---
                tensor = tf.nn.leaky_relu(
                    tf.layers.conv2d(tensor, n_filt, 4, 2, 'same', use_bias=use_bias, kernel_initializer=init_normal(),
                                     name='conv'))
                #tensor=tf.contrib.layers.layer_norm(tensor)
            with tf.variable_scope('extra_final.{0}-{1}'.format(tensor.shape[-1], 1)):
                tensor = tf.layers.conv2d(tensor, 10, 4, 1, 'same', use_bias=use_bias, kernel_initializer=init_normal(),
                                     name='conv')
                tensor = tf.layers.dense(tf.contrib.layers.flatten(tensor), units=self.n_class+1, use_bias=False, name='dense')
            return tensor
    ...
    def _build_loss(self, training=False, penalty_strength=10):
        ...
        fake = self._build_generator_contrib(self.input_z,self.input_z_y, training=training)
        ...
        #m_fake_logits=fake_logits*self.label_weights(self.input_z_y,source='fake')
        ...
        lossG = -tf.reduce_mean(fake_logits*self.label_weights(self.input_z_y,source='real',mode='label'))
        # discriminator loss
        with tf.name_scope('define_lossD') as scope:
            lossD_d = -tf.reduce_mean(real_logits*self.label_weights(self.input_y,source='real',mode='label'))
            lossD_g = tf.reduce_mean(fake_logits*self.label_weights(self.input_z_y,source='fake',mode='label'))
            norm_gradD = tf.sqrt(
        ...
        summary_noise=tf.random_normal([5*self.n_class, self.n_noise])
        summary_input=tf.convert_to_tensor(np.array(get_one_hot(np.repeat(np.arange(self.n_class),5),self.n_class),dtype=float),dtype=np.float32)
        summary_fake=self._build_generator_contrib(summary_noise,summary_input,training=False)
        if training:
...
if __name__ == '__main__':
    ...
    debug = FLAGS.debug
    epochs = FLAGS.ep
    wcat_gan = WCATGAN(debug=debug)
    start = time()
    wcat_gan.train(train_data, train_label, test_data, test_label, n_epochs=epochs, n_updated_d=5, learning_rate=1e-4)
