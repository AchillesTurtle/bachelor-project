...
def int_shape(x):
    ...
    # temporary solution for dynamic batch_size
    # s[0]=-1
    # return s
...
def deconv2d(x, num_filters, filter_size=[3, 3], stride=[1, 1], pad='SAME', nonlinearity=None, init_scale=1.,
    ...
    if pad == 'SAME':
        target_shape = tf.stack([batch_size, xs[1] * stride[0], xs[2] * stride[1], num_filters])
    else:
        target_shape = tf.stack(
            [batch_size, xs[1] * stride[0] + filter_size[0] - 1, xs[2] * stride[1] + filter_size[1] - 1, num_filters])
    with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
